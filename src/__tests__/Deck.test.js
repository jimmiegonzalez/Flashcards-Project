...
describe("Decks", () => {
  test("route for /decks/:deckId", async () => {
    ...
    const mockDeckPromise = Promise.resolve(mockDeck);
    readDeck.mockImplementation(() => mockDeckPromise);
  ...
  test("route for /decks/:deckId/cards/:cardId/edit", async () => {
    ...
    const mockCardPromise = Promise.resolve(cardTen);
    readCard.mockImplementation(() => mockCardPromise);
